...
/**
 * As of PHP 5.1, all Exceptions that happen at execution time should be
 * sub-classes of {@link http://www.php.net/~helly/php/ext/spl/classRuntimeException.html RunetimeException}.
 *
 * In order to account for this change, there is a RuntimeException defined
 * that will allow phpCR to run in PHP 5.0.x.  This code checks to make sure it
 * is necessary, prior to loading the exception file.
 */
if (!class_exists('RuntimeException')) {
	require_once PHPCR_PATH . '/exceptions/RuntimeException.exception.php';
}
