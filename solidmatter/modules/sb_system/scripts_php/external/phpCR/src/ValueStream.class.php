...
class ValueStream 
{
    ...
    public function stream_read($count)
    {
        ...
        $ret = substr($registry->$valueName, $this->_position, $count);
    ...
    public function stream_write($data)
    {
        ...
        $left = substr($registry->$valueName, 0, $this->_position);
        $right = substr($registry->$valueName, $this->_position + strlen($data));
    ...
    public function stream_eof()
    {
        ...
        return $this->_position >= strlen($registry->$valueName);
    }
    ...
    public function stream_seek($offset, $whence)
    {
        ...
        switch ($whence) {
        case SEEK_SET:
           if ($offset < strlen($registry->$valueName) && $offset >= 0) {
        ...
        case SEEK_END:
            if (strlen($registry->$valueName) + $offset >= 0) {
                $this->_position = strlen($registry->$valueName) + $offset;
                return true;
            } else {
                return false;
            }
...
stream_wrapper_register("Value", "ValueStream")
