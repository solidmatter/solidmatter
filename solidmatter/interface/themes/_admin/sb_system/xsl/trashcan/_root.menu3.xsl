...
<xsl:stylesheet 
	...
	<xsl:template name="render_content">
			...
			<xsl:otherwise>
				...
				<!--<a href="javascript:alert({@nodeid})">-->
					...
					<xsl:when test="count(./*) > 0">
						<img id="icon{$path}" src="modules/sb_system/themes/_admin/images_sysicons/tree_close.gif" name="close" />
					</xsl:when>
					<xsl:otherwise>
						<img id="icon{$path}" src="modules/sb_system/themes/_admin/images_sysicons/tree_open.gif" name="open" />
					</xsl:otherwise>
					</xsl:choose>
				</a>
			</xsl:otherwise>
		...
		<!-- label -->
		<xsl:choose>
			<xsl:when test="@csstype='root'">
				<a href="backend.view=welcome" class="type core" target="main"><xsl:value-of select="$locale/system/general/menu/root" /></a>
			</xsl:when>
			<xsl:otherwise>
