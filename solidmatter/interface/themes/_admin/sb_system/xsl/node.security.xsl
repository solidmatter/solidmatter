			...
			<tr>
				<td width="50%">
						...
						<ul class="invisible">
							<xsl:for-each select="groups/row">
								...
								<li style="margin-bottom:3px; display:block; position:relative;">
									<a href="/{$content/sbnode/@uuid}/security/editAuthorisations/?userentity={$id}" class="highlighted type sbSystem_Usergroup" target="details">
									...
									<xsl:if test="$content/sbnode/local_authorisations/authorisation[@uuid=$id]">
										<a href="/{$content/sbnode/@uuid}/security/removeUser/?userentity={$id}" class="type delete warning" style="position:absolute; right:0;">remove</a>
									</xsl:if>
								</li>
								</xsl:if>
							</xsl:for-each>
							<xsl:for-each select="users/row">
								...
								<xsl:if test="$content/sbnode/inherited_authorisations/authorisation[@uuid=$id] or $content/sbnode/local_authorisations/authorisation[@uuid=$id]">
								<li style="margin-bottom:3px; display:block; position:relative;">
									<a href="/{$content/sbnode/@uuid}/security/editAuthorisations/?userentity={$id}" class="highlighted type sbSystem_User" target="details">
					...
					<div class="eyecandy">
						...
						<form class="default" action="/{$content/sbnode/@uuid}/security/addUser" method="post" accept-charset="UTF-8">
							...
							<input style="margin-top:5px;" type="submit" value="{$locale/sbSystem/actions/add}" />
						</form>
					</div>
					<div class="eyecandy">
						<h1><xsl:value-of select="$locale/sbSystem/labels/security/inheritance" /></h1>
						<form class="default" action="/{$content/sbnode/@uuid}/security/changeInheritance" method="post" accept-charset="UTF-8">
							...
							<input style="margin-top:5px;" type="submit" value="{$locale/sbSystem/actions/save}" />
						</form>
					</div>
				</td>
